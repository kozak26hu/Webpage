...
async function getBalancerPrice() {
    try {
        ...
        const liquidityUSD = (Number(ethers.formatEther(reserveWETH)) * 2600) + (Number(ethers.formatEther(reservePHANDS)) * 0.023);
        if (liquidityUSD < 100) {
...
// Wallet connect with network switch and address display
async function connectWallet() {
    ...
    try {
        ...
        if (accounts.length > 0) {
            ...
            // Update the wallet-address element
            ...
            alert('No accounts found. Please unlock MetaMask or add an account.');
        }
    } catch (error) {
        if (error.code === 4001) {
            alert('You rejected the connection or network switch.');
        } else if (error.code === -32002) {
            alert('Pending request. Please approve in MetaMask.');
        } else if (error.code === 4902) {
            alert('Ethereum Mainnet not added. Please add it in MetaMask.');
        } else {
...
// Run price updates and timestamp on load and every 30 seconds
updateTimestamp();
updatePrices();
setInterval(() => {
    updateTimestamp();
